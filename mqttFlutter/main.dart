...
void pmqtt () async{
  var ord = await processo();
  print(ord);
}

  Future<List<String>> processo() async {
    var p = [];
  ...
  final MqttConnectMessage connMess = MqttConnectMessage()
    ...
    p.add('EXAMPLE::Mosquitto client connecting....');
  ...
  try {
    ...
    p.add('EXAMPLE::client exception - $e');
    client.disconnect();
  }

  /// Check we are connected
  if (client.connectionStatus.state == MqttConnectionState.connected) {
    p.add('EXAMPLE::Mosquitto client connected');
  } else {
